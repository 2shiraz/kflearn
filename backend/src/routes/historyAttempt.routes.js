...
import { requireBodyFields, validateAttemptMode } from "../validators/history.validators.js";
import { asyncHandler } from "../utils/asyncHandler.js";

const router = Router();

router.get("/", asyncHandler(listAttempts));
router.post("/", requireBodyFields(["moduleId", "mode"]), validateAttemptMode, asyncHandler(createAttempt));
router.get("/:attemptId", asyncHandler(getAttempt));
router.post("/:attemptId/messages", requireBodyFields(["text"]), asyncHandler(sendPatientMessage));
